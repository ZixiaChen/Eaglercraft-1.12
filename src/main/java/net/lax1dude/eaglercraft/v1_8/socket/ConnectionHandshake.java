...
public class ConnectionHandshake {
		...
		try {
			...
			if(type == HandshakePacketTypes.PROTOCOL_VERSION_MISMATCH) {
				...
				if(serverVers != protocolV2 && serverVers != protocolV3) {
					...
					mc.displayGuiScreen(new GuiDisconnected(ret, "connect.failed", new TextComponentString(serverVers < protocolV2 ? "Outdated Server" : "Outdated Client")));
					return false;
				}
				...
				if(gameVers != 47) {
					...
					mc.displayGuiScreen(new GuiDisconnected(ret, "connect.failed", new TextComponentString("This server does not support 1.8!")));
					return false;
				}
					...
					if(authType == HandshakePacketTypes.AUTH_METHOD_PLAINTEXT) {
						if(allowPlaintext) {
							...
							mc.displayGuiScreen(new GuiDisconnected(ret, "connect.failed",
									new TextComponentString(TextFormatting.RED + "Plaintext authentication was attempted but no user confirmation has been given to proceed")));
							return false;
						}
						...
						mc.displayGuiScreen(new GuiDisconnected(ret, "connect.failed",
								new TextComponentString(TextFormatting.RED + "Unsupported authentication type: " + authType + "\n\n" + TextFormatting.GRAY + "(Use a newer version of the client)")));
						return false;
					}
				}else {
					d.writeByte(0);
				...
				if(type == HandshakePacketTypes.PROTOCOL_SERVER_ALLOW_LOGIN) {
					...
					mc.displayGuiScreen(new GuiDisconnected(ret, "connect.failed", ITextComponent.Serializer.jsonToComponent(errStr)));
					return false;
				}else if(type == HandshakePacketTypes.PROTOCOL_SERVER_ERROR) {
					showError(mc, connecting, ret, di, serverVers == protocolV2);
					return false;
	...
	private static void showError(Minecraft mc, GuiConnecting connecting, GuiScreen scr, DataInputStream err, boolean v2) throws IOException {
		...
		if(errorCode == HandshakePacketTypes.SERVER_ERROR_RATELIMIT_BLOCKED) {
			...
			mc.displayGuiScreen(new GuiDisconnected(scr, "connect.failed", ITextComponent.Serializer.jsonToComponent(errStr)));
			...
			mc.displayGuiScreen(new GuiDisconnected(scr, "connect.failed", new TextComponentString("Server Error Code " + errorCode + "\n" + errStr)));
		}
	}
